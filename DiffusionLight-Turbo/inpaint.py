...
def create_argparser():    
    ...
    parser.add_argument("--resume", dest="resume", action="store_true", help="skip images/EVs/seeds that already have output files")
    parser.add_argument("--no_resume", dest="resume", action="store_false", help="disable output-file based resume")
...
def build_seed_output_names(image_path, outname, seed):
    if seed == "auto":
        filename = os.path.basename(image_path).split(".")[0]
        seed_value = name2hash(filename)
        outpng = f"{outname}.png"
        cache_name = f"{outname}"
    else:
        seed_value = int(seed)
        outpng = f"{outname}_seed{seed_value}.png"
        cache_name = f"{outname}_seed{seed_value}"

    return seed_value, outpng, cache_name

def file_ready(path):
    return os.path.exists(path) and os.path.getsize(path) > 0

def recover_square_from_raw(raw_path, square_path, x, y, r):
    if file_ready(raw_path) and not file_ready(square_path):
        os.makedirs(os.path.dirname(square_path), exist_ok=True)
        Image.open(raw_path).crop((x, y, x + r, y + r)).save(square_path)
        return True
    return False

def output_ready(raw_output_dir, square_output_dir, outpng, x, y, r):
    raw_path = os.path.join(raw_output_dir, outpng)
    square_path = os.path.join(square_output_dir, outpng)

    recover_square_from_raw(raw_path, square_path, x, y, r)

    return file_ready(raw_path) and file_ready(square_path)
    ...
    for image_id, image_data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        for ev, (prompt_embeds, pooled_prompt_embeds) in embedding_dict.items():
            ...
            for seed in seeds:
                ...
                start_time = time.time()
                seed_value, outpng, cache_name = build_seed_output_names(image_path, outname, seed)

                # Skip completed output when resuming. If raw exists but square is missing,
                # recover square by cropping raw instead of rerunning diffusion.
                if args.resume and output_ready(raw_output_dir, square_output_dir, outpng, x, y, r):
                    continue
                ...
                kwargs = {
                    ...
                    'mask_image': mask,
                    'strength': 1.0,
                    'current_seed': seed_value, # we still need seed in the pipeline!
