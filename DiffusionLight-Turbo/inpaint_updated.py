...
def create_argparser():    
    ...
    parser.add_argument("--seed", default="37", type=str, help="Seed: right now we use single seed instead to reduce the time, (Auto will use hash file name to generate seed)")
    ...
    parser.add_argument("--auto_ev", dest="auto_ev", action="store_true", help="adapt EV bracket per input image from scene luminance")
    parser.add_argument("--no_auto_ev", dest="auto_ev", action="store_false", help="disable adaptive EV and use --ev instead")
    parser.set_defaults(auto_ev=True)
    parser.add_argument("--auto_ev_count", default=3, type=int, help="number of EVs to generate when --auto_ev is enabled")
    parser.add_argument("--auto_ev_min_negative", default=-2.0, type=float, help="shallowest negative EV used by adaptive bracketing")
    parser.add_argument("--auto_ev_max_negative", default=-5.0, type=float, help="deepest negative EV used by adaptive bracketing")
    parser.add_argument("--auto_ev_gamma", default=2.4, type=float, help="gamma used to estimate input scene luminance for adaptive EV")
...
def interpolate_embedding_for_evs(pipe, args, ev_list):
    print("interpolate embedding...")
    interpolants = [ev / args.max_negative_ev for ev in ev_list]
...
def format_ev_for_filename(ev):
    ev_code = int(round(ev * 10))
    if ev_code == 0:
        return "-00"
    return str(ev_code)

def choose_adaptive_ev_list(input_image, args):
    image = np.asarray(input_image).astype(np.float32) / 255.0
    if image.ndim == 2:
        image = np.repeat(image[..., None], 3, axis=-1)
    image = image[..., :3]

    linear = np.power(np.clip(image, 0.0, 1.0), args.auto_ev_gamma)
    luminance = linear @ np.array([0.212671, 0.715160, 0.072169], dtype=np.float32)

    mean_luminance = float(np.mean(luminance))
    p99_luminance = float(np.percentile(luminance, 99))
    saturated_ratio = float(np.mean(luminance > 0.95))

    highlight_pressure = np.clip((p99_luminance - 0.75) / 0.22, 0.0, 1.0)
    saturation_pressure = np.clip(saturated_ratio / 0.01, 0.0, 1.0)
    brightness_pressure = np.clip((mean_luminance - 0.35) / 0.35, 0.0, 1.0)
    darkness_relief = np.clip((0.18 - mean_luminance) / 0.18, 0.0, 1.0)
    exposure_pressure = np.clip(
        0.45 * highlight_pressure + 0.35 * saturation_pressure + 0.20 * brightness_pressure - 0.35 * darkness_relief,
        0.0,
        1.0,
    )

    min_stop = abs(args.auto_ev_min_negative)
    max_stop = abs(args.auto_ev_max_negative)
    deepest_ev = -(min_stop + (max_stop - min_stop) * exposure_pressure)

    ev_values = np.linspace(0.0, deepest_ev, max(args.auto_ev_count, 2))
    ev_values = [round(float(ev) * 2.0) / 2.0 for ev in ev_values]
    ev_values[0] = 0.0
    ev_values[-1] = round(deepest_ev * 2.0) / 2.0
    ...
    print(
        "Adaptive EV stats:",
        f"mean={mean_luminance:.3f}",
        f"p99={p99_luminance:.3f}",
        f"sat={saturated_ratio:.4f}",
        "evs=" + ",".join(str(ev) for ev in deduped),
    )
    return deduped
...
def main():
    ...
    # interpolate embedding. Adaptive EV uses per-image embeddings inside the loop.
    embedding_dict = None if args.auto_ev else interpolate_embedding(pipe, args)
    ...
    for image_id, image_data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if args.auto_ev:
            image_ev_list = choose_adaptive_ev_list(input_image, args)
            image_embedding_dict = interpolate_embedding_for_evs(pipe, args, image_ev_list)
        else:
            image_embedding_dict = embedding_dict
        
        for ev, (prompt_embeds, pooled_prompt_embeds) in image_embedding_dict.items():
            # create output file name (we always use png to prevent quality loss)
            ev_str = format_ev_for_filename(ev)
